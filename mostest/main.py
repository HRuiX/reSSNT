...
class MOSTest:
    ...
    def _hook_fn(self, layer_name):
        def hook(module, inputs, outputs):
            assert len(inputs[0]) == outputs.shape[0], f"input has {len(inputs[0])}, output has {outputs.shape}."
            ...
            for idx, output in enumerate(tmp_outputs):
                if output.size()[0] != 1:
                    tmp_output = output.unsqueeze(0).mean((1, 2))
                else:
                    if len(output.size()) == 4 and not (isinstance(module, nn.Linear)):  # (N, K, H, w)
                        tmp_output = output.detach().mean((2, 3))
                    elif len(output.size()) == 4 and (isinstance(module, nn.Linear)):  # (N, K, H, w)
                        tmp_output = output.detach().mean((1, 2))
                    elif len(output.size()) == 3:  # (H, W, C)
                        tmp_output = output.detach().mean(dim=1)
                    else:
    ...
    def _evaluate_chromosome(self, population: List[Chromosome], datalist: Dict, desc: str) -> Chromosome:
        ...
        del results, use_dataloader_list
        gc.collect()
        torch.cuda.empty_cache()
    ...
    def build_ori_seed_prediction_cache(self, datalist, desc):
        ...
        with torch.no_grad():
            for idx in tqdm(range(0, len(datalist), self.max_items), desc=desc, leave=False):
                ...
                result = inference_model(self.model, datas)

                # 将当前batch的结果保存
                for i, cached_layer_dict in self.layer_output_input_dict.items():
    ...
    def build_muta_seed_prediction_cache(self, datalist, desc):
        ...
        with torch.no_grad():
            for idx in tqdm(range(0, len(datalist), self.max_items), desc=desc, leave=False):
                ...
                datas = [datalist[i][1] for i in idxs]
                seed_idx = [datalist[i][0] for i in idxs]
                ...
                for i, cached_layer_dict in self.layer_output_input_dict.items():
                    pred_sem_seg = result[i].pred_sem_seg.data.cpu().numpy()
                    self.muta_seed_prediction_cache[seed_idx[i]] = (cached_layer_dict, pred_sem_seg.copy())
    ...
    def get_prediction_with_cache(self, seed_idx, seed_prediction_cache):
