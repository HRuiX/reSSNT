    ...
    for model_type_idx in [1, 2, 3]:
        ...
        for dataset in DATASETS[datasets_idx]:
            ...
            for model_info in model_infos:
                ...
                # console.print(
                #     Text(f"Step 3. Choice the {testTool.choice_samples} files of val dataset (bottom/top/random).",
                #          style="bold green"))
                # testTool.select_files()
                #
                # # Step 4: 对3个进行覆盖度量的计算
                # console.print(Text(f"Step 4. Calculate coverage metrics for three {testTool.choice_samples}s.",
                #                    style="bold green"))
                ...
                # console.print(
                #     Text(f"Step 5. Calculate diversity for three {testTool.choice_samples}s", style="bold green"))
                ...
                # analyse.analysis_iou([], testTool.data_save_path_prefix, testTool.model_name,
                #                      testTool.coverages_setting, cov_select_type=testTool.cov_select_type)
